...
export class CartComponent implements OnInit {
  cartIdControl: UntypedFormControl = new UntypedFormControl('', [Validators.required]);
    ...
    this.apiService.getAllProducts().subscribe((products) => {
      ...
      if (this.authService.loggedUser) {
        ...
        this.apiService.getProductsByCartId(cartId).subscribe((response) => {
          ...
          this.productsNotInCart.forEach((p) => {
            p.quantity = 0;
          })
  ...
  minusQuantity(id: string) {
    this.productsNotInCart.forEach((p) => {
      if (p.quantity !== undefined && p.quantity !== 0 && p.id === id) {
        p.quantity -= 1;
      }
    })
  }
  ...
  addQuantity(id: string) {
    this.productsNotInCart.forEach((p) => {
      if (p.quantity !== undefined && p.id === id) {
        p.quantity += 1;
      }
    })
  }
